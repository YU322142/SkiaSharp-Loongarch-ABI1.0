...
SYSROOT_URL="${SYSROOT_URL:-https://github.com/YU322142/SkiaSharp-Loongarch-ABI1.0/releases/download/oldworld-dev-sysroot-20260607/loongarch64-oldworld-dev-sysroot-overlay-20260607.tar.xz}"
...
write_gn_args() {
  ...
  ar_gn="$(to_gn_path "$AR")"
  map_gn="$(to_gn_path "$SRC_DIR/native/linux/libSkiaSharp/libSkiaSharp.map")"
...
extra_cflags=[ "--sysroot=$sysroot_gn", "-I$sysroot_gn/usr/include", "-I$sysroot_gn/usr/include/freetype2", "-DSKIA_C_DLL", "-DHAVE_SYSCALL_GETRANDOM", "-DXML_DEV_URANDOM" ]
extra_asmflags=[]
...
build_skiasharp() {
  ...
  cp -f "$so" "$OUT_DIR/libSkiaSharp.so"
  cp -f "$so" "$OUT_DIR/libSkiaSharp.so.119.0.0"
